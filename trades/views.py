    ...
    def post(self, request):
        ...
        if trade_serializer.is_valid():
            try:
                ...
                return Response(
                    {"error": "Stock not found"}, status=status.HTTP_400_BAD_REQUEST
                )
            try:
                portfolio = Portfolio.objects.get(id=portfolio_id)
            except Portfolio.DoesNotExist:
                return Response(
                    {"error": "Portfolio not found"}, status=status.HTTP_400_BAD_REQUEST
                )

            if (
                portfolio.portfolio_value == None
                and (
                    (-1 if trade_data["trade_type"] == "Sell" else 1)
                    * trade_data["amount"]
                    * trade_data["stock_price"]
                )
                >= 0
            ) or portfolio.portfolio_value + (
                (-1 if trade_data["trade_type"] == "Sell" else 1)
                * trade_data["amount"]
                * trade_data["stock_price"]
            ) >= 0:
                ...
                return Response(
                    {
                        "error": "Portfolio value cannot be less than 0 after adding the new trade"
                    },
...
class StockAPIView(APIView):
    ...
    def delete(self, request):
        ...
        stock.delete()
        return Response(
            {"message": f"Stock with Name {pk} deleted successfully"},
            status=status.HTTP_200_OK,
        )
...
class PortfolioAPIView(APIView):
    ...
    def delete(self, request):
        ...
        portfolio.delete()
        return Response(
            {"message": f"Portfolio with id {pk} deleted successfully"},
            status=status.HTTP_200_OK,
        )
...
class FetchTradeData(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response(
            {"trade_information": serializer.data, "trade_current_price": current_price}
        )
